...
<template>
  <div class="newsContent">
    <div :title="newsTitle" class="newsDetailsTitle text-title mt-2 pl-2 pr-2 ms-2">
      {{ newsTitle }}
    </div>
    <div v-if="currentUser" class="d-flex flex-row pa-2 ms-2">
      ...
      <v-list-item-title v-if="space" class="text-subtitle mb-0">
        <exo-space-avatar
          :space="space"
          :size="32"
          class="align-center my-auto text-truncate flex-grow-0 flex"
    ...
    <div class="d-flex flex-row text-subtitle">
      <exo-news-details-time :news="news" />
    </div>
    <div class="d-flex flex-row text-header pa-4">
      <span v-sanitized-html="newsSummary"></span>
    </div>
    <v-divider class="mx-4" />
    <div class="d-flex flex-column pa-4 newsBody">
      <div
