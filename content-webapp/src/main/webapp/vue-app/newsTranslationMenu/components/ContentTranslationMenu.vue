  ...
  <v-menu
    ...
    <template #activator="{ on, attrs }">
      <v-icon
        ...
        class="ms-3 remove-focus my-auto pa-0  pe-1"
        v-bind="attrs"
        v-on="on">
        fa-language
      </v-icon>
...
export default {
  ...
  methods: {
    refreshTranslationExtensions() {
      this.menuExtensions = extensionRegistry.loadExtensions('news', 'translation-menu-extension');
    },
    changeTranslation(item) {
      this.$root.$emit('change-article-translation', item);
    }
  }
};
</script>
