...
export default {
  ...
  computed: {
    ...
    disabled() {
      return (this.showSeeAll && !this.isValidSeeAllUrl)
          || (this.isTargetSourceOption && !this.newsTarget)
          || (this.isSelectedListSourceOption && !this.selectedArticles?.length)
          || this.limitExceeded;
    },
    previewTemplate() {
  ...
  methods: {
    ...
    reset() {
      ...
      this.showArticleSummary = this.viewTemplate === 'NewsAlert' || this.viewTemplate === 'NewsMosaic' || this.viewTemplate === 'NewsStories' ? false : this.$root.showArticleSummary;
