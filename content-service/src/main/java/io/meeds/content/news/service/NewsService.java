...
public class NewsService {
  ...
  // required = false + null guard: the bean is a social @Service reached
  // through the cross-WAR bridge at runtime (content boots after social), but
  // it is absent from this module's own Spring test contexts, which scan a
  // closed list of packages (same pattern as the App Center badge
  // contributors, see eng-standards extensibility norm)
  ...
  private void markArticleReportsStale(News news) {
    if (activityReportService == null || StringUtils.isBlank(news.getActivityId())) {
      return;
    }
    ExoSocialActivity activity = activityManager.getActivity(news.getActivityId());
    if (activity != null) {
