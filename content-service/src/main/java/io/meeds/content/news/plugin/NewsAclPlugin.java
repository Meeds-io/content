...
@Component
public class NewsAclPlugin implements AclPlugin {

  private static final String OBJECT_TYPE = NewsPermanentLinkPlugin.OBJECT_TYPE;
  ...
  public boolean hasPermission(String objectId, String permissionType, Identity identity) {
    ...
    if (news == null) {
      ...
      case VIEW_PERMISSION_TYPE: {
        yield newsService.canViewNews(news, identity == null ? null : identity.getUserId());
      }
      case EDIT_PERMISSION_TYPE: {
        yield newsService.canEditNews(news, identity == null ? null : identity.getUserId());
      }
      default:
        yield false;
      };
